...
export const SupplierPaymentsPage: React.FC = () => {
  ...
  const handlePrintSlip = async () => {
    ...
    try {
      await printDocument({
        elementOrId: 'payment-voucher-slip-content',
        title: `Payment Voucher - ${viewVoucher.paymentNumber}`,
        filename: `Payment_${viewVoucher.paymentNumber}`,
      ...
      {viewVoucher && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs overflow-y-auto">
          <div className="bg-white rounded-2xl max-w-2xl w-full border border-slate-200 shadow-2xl overflow-hidden my-8">
            ...
            <div id="payment-voucher-slip-content" className="p-6 space-y-6">
              {/* Slip Header Box */}
              ...
              <div className="flex items-center justify-between pt-4 border-t border-slate-100">
                ...
                >
                  <Printer className={`w-4 h-4 ${isPrintingSlip ? 'animate-pulse text-blue-600' : ''}`} />
                  {isPrintingSlip ? 'Preparing Payment Slip...' : 'Print Payment Slip'}
                </button>
