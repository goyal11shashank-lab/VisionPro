...
export const SalesInvoicesPage: React.FC<{
  ...
  const handleOpenPrintPreview = async (inv: any, autoPrint = false) => {
    setAutoInvokePrint(autoPrint);
    if (!inv.lines || inv.lines.length === 0) {
      try {
        ...
        if (res.ok) {
          ...
          setPrintPreviewInvoice(full || inv);
        } else {
          setPrintPreviewInvoice(inv);
        }
      } catch {
        setPrintPreviewInvoice(inv);
      }
  ...
  const handleExportInvoicesExcel = () => {
    const columns: ExcelColumn[] = [
      ...
      { header: 'State', key: 'partyState', width: 16 },
      { header: 'Source Order', key: 'salesOrderNumber', width: 16 },
    ...
    exportToExcel({
      filename: `sales_invoices_${new Date().toISOString().split('T')[0]}`,
      sheetName: 'Sales Invoices',
      reportTitle: 'Sales Invoices Register',
      businessName: currentBusiness?.name,
      filtersSummary: `Status: ${statusFilter}${search ? `, Search: "${search}"` : ''} | Total: ${invoices.length} invoices`,
      columns,
      data: invoices,
    });
  };
  ...
  const fetchInvoices = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errData.message || 'Failed to load sales invoices');
      }
  ...
  const promptDeleteInvoice = (inv: { id: string; invoiceNumber: string; status: string; partyName?: string; grandTotal?: string | number }) => {
    setDeleteError(null);
    setInvoiceToDelete(inv);
  };

  // Confirm Delete Invoice Execution
  const handleConfirmDeleteInvoice = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/sales/invoices/${invoiceToDelete.id}`, {
        method: 'DELETE',
        headers: getAuthHeaders(currentBusiness?.id),
      });

      const data = await res.json().catch(() => ({}));

      if (!res.ok) {
        throw new Error(data.message || data.error || 'Failed to delete sales invoice');
      }
      ...
      setNotification({
        type: 'success',
        message: data.message || `Sales Invoice #${invoiceToDelete.invoiceNumber} deleted successfully.`,
      });
      fetchInvoices();
    } catch (err: any) {
  ...
  // Open Cancel Modal
  const promptCancelInvoice = (inv: { id: string; invoiceNumber: string }) => {
    setCancelReason('');
    setCancelError(null);
    setInvoiceToCancel(inv);
  };

  // Confirm Cancel Invoice Execution
  const handleConfirmCancelInvoice = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/sales/invoices/${invoiceToCancel.id}/cancel`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          ...getAuthHeaders(currentBusiness?.id),
        },
      ...
      if (!res.ok) {
        throw new Error(data.message || data.error || 'Failed to cancel sales invoice');
      }
      ...
      setNotification({
        type: 'success',
        message: `Sales Invoice #${invoiceToCancel.invoiceNumber} cancelled successfully. Stock and ledger reversed.`,
      });
      fetchInvoices();
    } catch (err: any) {
      setCancelError(err.message || 'Failed to cancel sales invoice');
    } finally {
  ...
  const handleDeleteInvoice = (invoiceId: string, invoiceNumber: string, status: string = 'POSTED', partyName?: string, grandTotal?: string | number) => {
    promptDeleteInvoice({ id: invoiceId, invoiceNumber, status, partyName, grandTotal });
  };

  const handleCancelInvoice = (invoiceId: string, invoiceNumber?: string) => {
    promptCancelInvoice({ id: invoiceId, invoiceNumber: invoiceNumber || selectedInvoice?.invoiceNumber || '' });
  };
  ...
  return (
    <div id="sales-invoices-container" className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      {notification && (
        <div
          id="sales-invoice-notification-banner"
          className={`p-4 rounded-xl border flex items-center justify-between gap-3 shadow-sm transition ${
        ...
        >
          ...
          <button
            ...
            className="text-xs font-semibold opacity-70 hover:opacity-100 underline ml-auto"
          >
            Dismiss
          </button>
        </div>
      ...
      <div className="flex flex-col sm:flex-row items-center gap-4 bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          <div className="flex items-center gap-1.5 border-l border-slate-200 pl-2">
            <button
              id="btn-export-sales-invoices-excel"
              ...
              title="Export filtered sales invoices to Excel (.xlsx)"
            >
              <FileSpreadsheet className="w-3.5 h-3.5" />
              <span>Excel</span>
            </button>
            <button
              id="btn-export-sales-invoices-csv"
              ...
              title="Export filtered sales invoices to CSV"
            >
                  ...
                  <tr key={inv.id} className="hover:bg-slate-50 transition">
                    ...
                    <td className="py-3.5 px-4 text-center">
                      <div className="flex items-center justify-center gap-1">
                        ...
                        <button
                          id={`btn-print-preview-invoice-${inv.id}`}
                          onClick={() => {
                            const previewBeforePrint = settings?.print?.previewBeforePrint ?? true;
                            handleOpenPrintPreview(inv, !previewBeforePrint);
                          }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-5xl my-8 overflow-hidden border border-slate-300">
            <div className="px-6 py-4 bg-slate-100 border-b border-slate-300 flex items-center justify-between">
              <div>
                <h3 className="text-lg font-black text-slate-950">Direct Sales Tax Invoice</h3>
                <p className="text-xs text-slate-600 font-bold">
                  Target Number: <span className="font-mono text-emerald-800 font-black">{previewInvoiceNumber || 'Auto-generated'}</span>
                </p>
              </div>
              <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 text-sm rounded-lg border border-slate-300 font-medium text-slate-900 focus:ring-2 focus:ring-emerald-500 focus:outline-none"
                  >
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl overflow-hidden border border-slate-200">
            ...
            <div className="px-6 py-3.5 bg-slate-900 text-white flex items-center justify-between print:hidden">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  id="btn-print-preview-modal-action"
                  onClick={() => {
                    setAutoInvokePrint(false);
                    setPrintPreviewInvoice(selectedInvoice);
                    setIsPrintPreviewOpen(true);
                  }}
                  className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 hover:bg-blue-500 text-xs font-semibold rounded-lg transition text-white shadow-xs"
                ...
                <button
                  id="btn-direct-print-modal-action"
                  onClick={() => {
                    const previewBeforePrint = settings?.print?.previewBeforePrint ?? true;
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-xs font-semibold rounded-lg transition text-slate-200"
                  title="Print Invoice"
                >
                  <Printer className="w-3.5 h-3.5" />
                  Print
              ...
              <div className="border border-slate-200 rounded-xl overflow-hidden">
                <table className="w-full text-left text-xs">
                  ...
                  <tbody className="divide-y divide-slate-100 font-mono">
                    {isDetailLoading && (!selectedInvoice.lines || selectedInvoice.lines.length === 0) ? (
                      <tr>
                        <td colSpan={6} className="py-8 text-center text-slate-500 font-sans">
                          <div className="flex items-center justify-center gap-2">
                            <RefreshCw className="w-4 h-4 animate-spin text-emerald-600" />
                            <span>Loading items sold &amp; batch allocations...</span>
                          </div>
                        </td>
                      </tr>
                      ...
                      <tr>
                        <td colSpan={6} className="py-8 text-center text-slate-400 font-sans italic">
                          No items sold recorded on this invoice.
                        </td>
                      </tr>
                    ) : (
                      (selectedInvoice.lines || []).map((l, idx) => {
                        const itemName = l.uniqueItemName || (l as any).uniqueItem?.name || 'Optical Item';
                        const itemCode = l.uniqueItemCode || (l as any).uniqueItem?.code;
                        ...
                        return (
                          <tr key={l.id || idx}>
                                    ...
                                    return (
                                      <div key={bIdx} className="flex flex-wrap items-center gap-x-2 bg-slate-50 p-1 rounded border border-slate-200/60 text-slate-700">
                                        <span className="font-semibold text-slate-900">
                                          Power: SPH {sph}, CYL {cyl}{axis ? `, AXIS ${axis}` : ''}{add ? `, ADD ${add}` : ''}
                                        </span>
                                        {side && side !== 'NONE' && (
                            ...
                            <td className="py-3 px-3 text-right">₹{rate.toFixed(2)}</td>
                            <td className="py-3 px-3 text-right">₹{taxable.toFixed(2)}</td>
              ...
              <div className="flex items-center gap-2">
                ...
                {selectedInvoice.status === 'POSTED' && (
                  <button
                    ...
                    onClick={() => promptCancelInvoice(selectedInvoice)}
                ...
                <button
                  ...
                  onClick={() => promptDeleteInvoice(selectedInvoice)}
                  disabled={deleteLoading || cancelLoading}
      ...
      {invoiceToDelete && (
        <div id="modal-delete-invoice" className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl border border-slate-200 max-w-md w-full overflow-hidden animate-in fade-in zoom-in-95 duration-150">
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">
                    Permanent action — please review before proceeding
                  </p>
                </div>
              </div>

              <div className="mt-4 p-3.5 bg-slate-50 rounded-lg border border-slate-200 text-xs space-y-2">
                <div className="flex justify-between items-center">
                  <span className="text-slate-500">Invoice Number:</span>
                  <span className="font-mono font-bold text-slate-900">{invoiceToDelete.invoiceNumber}</span>
                </div>
                {invoiceToDelete.partyName && (
                  <div className="flex justify-between items-center">
                    <span className="text-slate-500">Customer:</span>
                    <span className="font-medium text-slate-800 truncate max-w-[200px]">{invoiceToDelete.partyName}</span>
                  </div>
                )}
                {invoiceToDelete.grandTotal !== undefined && (
                  <div className="flex justify-between items-center">
                    <span className="text-slate-500">Invoice Amount:</span>
                    <span className="font-mono font-bold text-slate-900">
                      ₹{parseFloat(String(invoiceToDelete.grandTotal)).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                    </span>
                  </div>
                )}
                <div className="flex justify-between items-center">
                  <span className="text-slate-500">Current Status:</span>
                  <span className="font-semibold text-slate-700">{invoiceToDelete.status}</span>
                </div>
              </div>

              <div className="mt-3 p-3 bg-amber-50 rounded-lg border border-amber-200 text-xs text-amber-800">
                <p className="font-medium">
                  {invoiceToDelete.status === 'POSTED'
                    ? '⚠️ Deleting this posted invoice will permanently reverse all associated inventory stock deductions and customer ledger debit postings.'
                    : invoiceToDelete.status === 'CANCELLED'
                    ? 'ℹ️ Deleting this cancelled invoice will permanently purge the record from the register.'
                    : 'ℹ️ Deleting this draft invoice will permanently remove it from the system.'}
                </p>
              </div>
            ...
            <div className="px-6 py-4 bg-slate-50 border-t border-slate-200 flex items-center justify-end gap-3">
              <button
                type="button"
                id="btn-cancel-delete-invoice"
                onClick={() => setInvoiceToDelete(null)}
                disabled={deleteLoading}
                className="px-4 py-2 bg-white hover:bg-slate-100 text-slate-700 text-xs font-semibold rounded-lg border border-slate-300 transition"
              >
                Cancel
              </button>
              <button
                ...
                disabled={deleteLoading}
                className="px-4 py-2 bg-rose-600 hover:bg-rose-700 disabled:opacity-50 text-white text-xs font-semibold rounded-lg shadow transition flex items-center gap-1.5"
              >
                {deleteLoading ? (
                  <>
      ...
      {invoiceToCancel && (
        <div id="modal-cancel-invoice" className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl border border-slate-200 max-w-md w-full overflow-hidden animate-in fade-in zoom-in-95 duration-150">
            <div className="p-6">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="text-lg font-bold text-slate-900">Cancel Sales Invoice</h3>
                  <p className="text-xs text-slate-500 mt-0.5">
                    Invoice #{invoiceToCancel.invoiceNumber}
                  </p>
                </div>
              </div>
              ...
              <div className="mt-4">
                ...
                <textarea
                  id="input-cancel-invoice-reason"
                  rows={3}
                  value={cancelReason}
                  onChange={e => setCancelReason(e.target.value)}
                  placeholder="State the reason for cancellation (required for audit logs & ledger reversal)..."
            ...
            <div className="px-6 py-4 bg-slate-50 border-t border-slate-200 flex items-center justify-end gap-3">
              ...
              <button
                ...
                className="px-4 py-2 bg-amber-600 hover:bg-amber-700 disabled:opacity-50 text-white text-xs font-semibold rounded-lg shadow transition flex items-center gap-1.5"
              >
                {cancelLoading ? (
                  <>
                    <span className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
      ...
      {isPrintPreviewOpen && printPreviewInvoice && (
        <PrintPreviewModal
          ...
          title={`Tax Invoice - ${printPreviewInvoice.invoiceNumber || ''}`}
          filename={`Invoice_${printPreviewInvoice.invoiceNumber || 'INV'}`}
