...
export const SalesReturnsPage: React.FC = () => {
  ...
  const fetchReturns = async () => {
    try {
      ...
      const res = await apiRequest<{ items: SalesReturn[]; total: number }>(
  ...
  // Open Create Return Dialog: Fetch POSTED invoices
  const handleOpenCreateModal = async () => {
    ...
    try {
      ...
      const res = await apiRequest<{ items: any[] }>(`/api/sales/invoices?status=POSTED&limit=50`);
      setInvoicesList(res.items || []);
    } catch (err: any) {
      setErrorMsg('Failed to load posted sales invoices: ' + err.message);
    } finally {
      setLoadingInvoices(false);
    }
  };

  // Select Invoice & Load Summary
  const handleSelectInvoice = async (invoiceId: string) => {
    setSelectedInvoiceId(invoiceId);
    setErrorMsg('');
    try {
      setLoadingSummary(true);
      const data = await apiRequest<any>(`/api/sales/returns/invoice-summary/${invoiceId}`);
      ...
      const initializedLines = (data.lines || []).map((l: any) => ({
        ...
        batches: (l.batches || []).map((b: any) => ({
          ...
          invoicedQty: b.invoicedQuantity,
      ...
      setErrorMsg('Failed to load invoice items: ' + err.message);
    } finally {
  ...
  const handleCreateSubmit = async (shouldPost: boolean) => {
    ...
    // Verify batch quantity matches line quantity
    ...
    try {
      ...
      await apiRequest('/api/sales/returns', {
        method: 'POST',
  ...
  const handleViewDetails = async (id: string) => {
    try {
      ...
      const data = await apiRequest<any>(`/api/sales/returns/${id}`);
      setViewReturn(data);
    } catch (err: any) {
  ...
  const handlePostReturn = async (id: string) => {
    if (!confirm('Are you sure you want to POST this Sales Return? This will restore stock to the original batches and credit the customer ledger.')) {
      return;
    }
    try {
      await apiRequest(`/api/sales/returns/${id}/post`, { method: 'POST' });
      fetchReturns();
      if (viewReturn?.id === id) {
        handleViewDetails(id);
      }
    } catch (err: any) {
      alert('Failed to post sales return: ' + err.message);
    }
  };
  ...
  const handleCancelSubmit = async () => {
    ...
    try {
      setCancelling(true);
      await apiRequest(`/api/sales/returns/${cancelReturnId}/cancel`, {
        method: 'POST',
      ...
      alert('Failed to cancel sales return: ' + err.message);
    } finally {
      setCancelling(false);
    }
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900 border border-slate-800 p-6 rounded-2xl">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400">
                Process customer optical returns, restore original batch inventory, and issue GST credit notes.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-500 text-white font-medium text-sm transition-colors shadow-lg shadow-rose-600/20"
          >
          ...
          <table className="w-full text-left border-collapse text-sm">
            ...
            <tbody className="divide-y divide-slate-800/60 text-slate-300">
              {loading ? (
                ...
                <tr>
                  <td colSpan={8} className="py-12 text-center text-slate-500">
                    No sales returns found matching current filters.
                  </td>
                </tr>
              ) : (
                returns.map(ret => (
                  <tr key={ret.id} className="hover:bg-slate-800/40 transition-colors">
                    ...
                    <td className="py-3.5 px-4">
                      <div className="font-medium text-white">{ret.party?.name || 'Customer'}</div>
                      <div className="text-xs text-slate-500">{ret.party?.phone || ret.party?.city}</div>
                    </td>
          ...
          <div className="bg-slate-900 border border-slate-800 w-full max-w-4xl rounded-2xl overflow-hidden shadow-2xl my-8">
            <div className="p-6 border-b border-slate-800 flex items-center justify-between bg-slate-950/60">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-white text-lg">Create Sales Return (Credit Note)</h3>
                  <p className="text-xs text-slate-400">
                    Select a posted sales invoice and specify items/batches to return.
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="p-6 space-y-6 max-h-[75vh] overflow-y-auto">
              ...
              <div>
                ...
                {loadingInvoices ? (
                  <div className="text-sm text-slate-400 py-3">Loading posted invoices...</div>
                ) : (
                  <select
                    value={selectedInvoiceId}
                    onChange={e => handleSelectInvoice(e.target.value)}
                    className="w-full px-4 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-sm text-white focus:outline-none focus:border-rose-500"
                  >
                    <option value="">-- Choose Posted Sales Invoice --</option>
                    {invoicesList.map(inv => (
                      <option key={inv.id} value={inv.id}>
                        {inv.invoiceNumber} — {inv.party?.name || 'Customer'} — ₹{parseFloat(inv.grandTotal).toLocaleString('en-IN')} ({new Date(inv.invoiceDate).toLocaleDateString()})
                      </option>
                    ))}
                  </select>
                )}
                ...
                <div className="space-y-6">
                  <div className="p-4 bg-slate-950/70 border border-slate-800 rounded-xl grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs">
                    <div>
                      ...
                      <div className="font-semibold text-white text-sm mt-0.5">{invoiceSummary.party?.name}</div>
                    </div>
                    <div>
                      <div className="text-slate-500 uppercase">Invoice Date</div>
                      <div className="font-medium text-slate-300 mt-0.5">
                        {new Date(invoiceSummary.invoice?.invoiceDate).toLocaleDateString()}
                      </div>
                    </div>
                    <div>
                      <div className="text-slate-500 uppercase">Original Amount</div>
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-2">
                      2. Return Item & Original Batch Allocations *
                    </label>
                        ...
                        >
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-800/80 pb-2">
                            ...
                            <div className="text-xs text-slate-400">
                              Invoiced: <strong className="text-slate-200">{line.invoicedQty}</strong> | Max Returnable:{' '}
                              <strong className="text-rose-400">{line.returnableQty}</strong>
                            </div>
                          </div>
                          ...
                          <div className="space-y-2 pt-1">
                            <div className="text-xs font-medium text-slate-400 flex items-center gap-1.5">
                              ...
                              Original Batches for this Item:
                            </div>
                              ...
                              >
                                <div>
                                  ...
                                  <span className="text-slate-500 ml-2">
                                    (Max Returnable: {b.returnableQty})
                                  </span>
                                </div>
                                <div className="flex items-center gap-2">
                                  ...
                                  <input
                                    ...
                                    className="w-20 px-2 py-1 bg-slate-950 border border-slate-700 rounded text-center text-white font-mono focus:outline-none focus:border-rose-500"
                                  />
                                </div>
                              </div>
                            ))}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        type="text"
                        placeholder="e.g. Frame size misfit, customer power revised..."
                        value={returnReason}
                  ...
                  <div className="p-4 bg-slate-950/80 border border-slate-800 rounded-xl flex flex-col sm:flex-row justify-between items-center gap-4">
                    ...
                    <div className="text-right">
                      <div className="text-xs text-slate-500 uppercase tracking-wider">Credit Note Total</div>
            ...
            <div className="p-6 border-t border-slate-800 bg-slate-950/80 flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-5 py-2 rounded-xl bg-rose-600 hover:bg-rose-500 text-white text-sm font-semibold transition-colors disabled:opacity-50 shadow-lg shadow-rose-600/20"
                >
            ...
            <div className="p-6 border-b border-slate-800 flex items-center justify-between bg-slate-950">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">
                    Against Invoice {viewReturn.invoice?.invoiceNumber} • Created{' '}
            ...
            <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-4 bg-slate-950 rounded-xl text-xs">
                <div>
                  ...
                  <div className="font-medium text-white mt-0.5">{viewReturn.party?.name}</div>
                </div>
                <div>
                  <span className="text-slate-500 uppercase">Status</span>
                  <div className="mt-0.5 font-semibold text-rose-400">{viewReturn.status}</div>
                </div>
                <div>
                ...
                <div>
                  ...
                  <div className="font-mono font-bold text-rose-400 mt-0.5">₹{viewReturn.grandTotal}</div>
                </div>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-800 w-full max-w-md rounded-2xl p-6 shadow-2xl space-y-4">
            <div className="flex items-center gap-3 text-rose-400">
              ...
              <h3 className="text-lg font-bold text-white">Cancel Sales Return</h3>
            </div>
            <p className="text-xs text-slate-400">
              Are you sure? If posted, this will deduct restored stock from original batches and debit the customer ledger back.
            </p>
            <div>
